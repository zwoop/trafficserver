...
struct CacheProcessor : public Processor {
  ...
  Action *lookup(Continuation *cont, const CacheKey *key, int volume, CacheFragType frag_type = CACHE_FRAG_TYPE_NONE,
                 const char *hostname = nullptr, int host_len = 0);
  Action *open_read(Continuation *cont, const CacheKey *key, int volume, CacheFragType frag_type = CACHE_FRAG_TYPE_NONE,
                    const char *hostname = nullptr, int host_len = 0);
  Action *open_write(Continuation *cont, CacheKey *key, int volume, CacheFragType frag_type = CACHE_FRAG_TYPE_NONE,
                     int expected_size = CACHE_EXPECTED_SIZE, int options = 0, time_t pin_in_cache = 0, char *hostname = nullptr,
                     int host_len = 0);
  Action *remove(Continuation *cont, const CacheKey *key, int volume, CacheFragType frag_type = CACHE_FRAG_TYPE_NONE,
                 const char *hostname = nullptr, int host_len = 0);
  Action *scan(Continuation *cont, char *hostname = nullptr, int host_len = 0, int KB_per_second = SCAN_KB_PER_SECOND);
  Action *lookup(Continuation *cont, const HttpCacheKey *key, int volume, CacheFragType frag_type = CACHE_FRAG_TYPE_HTTP);
  ...
  Action *open_write(Continuation *cont, int expected_size, const HttpCacheKey *key, CacheHTTPHdr *request, CacheHTTPInfo *old_info,
                     int volume, time_t pin_in_cache = 0, CacheFragType frag_type = CACHE_FRAG_TYPE_HTTP);
  Action *remove(Continuation *cont, const HttpCacheKey *key, int volume, CacheFragType frag_type = CACHE_FRAG_TYPE_HTTP);
